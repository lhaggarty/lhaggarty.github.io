...
if(typeof window.twttr!="undefined"){window.twttr.ready(function(a){a.events.bind("tweet",function(b){if(b){if(b.target&&b.target.nodeName=="IFRAME"){twitterButton()
}}})
})
}});
...
$(document).on("star_component_init",function(){$("div.more-stories-bar p.switch, div.more-stories-bar span.close-button").on("click",function(){var a=200;
var c=$(this).closest("div.homepage-top");
if(typeof c.attr("data-switching")=="undefined"){c.attr("data-switching","true");
var b=c.find("div.more-stories-bar p.switch").html();
if(!c.hasClass("switched")){c.addClass("switched");
c.find(".more-headlines").fadeOut(a);
c.find(".headlines:not(.more-stories-panel)").fadeOut(a,function(){c.find(".headlines.more-stories-panel").fadeIn(a);
c.removeAttr("data-switching")
});
trackHomePageTopFlip(b,"open")
}else{c.removeClass("switched");
c.find(".headlines.more-stories-panel").fadeOut(a,function(){c.find(".more-headlines").fadeIn(a);
c.find(".headlines:not(.more-stories-panel)").fadeIn(a,function(){c.removeAttr("data-switching")
})
});
trackHomePageTopFlip(b,"close")
}}})
});
...
$(".labelcomment").click(function(){var d=$("#comments-section").offset();
$("html, body").animate({scrollTop:d.top},"slow")
});
$(function(){$(".text-decrease,.text-increase,.print-article,.email-article,.explore-menu,.labelcomment").hover(function(f){var d=$(this).attr("title");
...
$(document).ready(function(){if($("body.type-story").length&&$("#google-multivar").length&&(typeof currentState!="undefined"&&currentState.toLowerCase()=="disabled")){var d=$("<div/>").addClass("multivar float-clear").append($("<p/>").append(" Get breaking news stories sent to your inbox as they happen.")).append($("<a/>").addClass("button").attr("href","http://www3.thestar.com/static/newsletter/email_registration.html").append("Sign Up").click(function(){_gaq.push(["_trackEvent","ArticleMultivarTest","ClickedCTA"])
}));
if(_star_qs_params.cta){if(_star_qs_params.cta=="bottom"){$(".story-footer").prepend(d.addClass("bottom"))
}else{if(_star_qs_params.cta=="right"){$(".right-rail > .toprightrail").append(d.addClass("right"))
}else{if(_star_qs_params.cta=="explore"){$(".article-story-body").before(d.addClass("explore"))
}}}}}if($(".article-story-body").length){var j=$.cookie("article-font-size");
if(j!=null){$(".article-story-body").css("font-size",j+"px")
}}if($("#explore-comments-count").length){var h="https://api.viafoura.com/v2/"+window.location.host;
var g=encodeURIComponent(window.location.pathname);
$.ajax({url:h+"/pages/"+g+"?verb=get",async:false,type:"GET",dataType:"jsonp",success:function(l){$("#explore-comments-count , .commentCount").text(l.result.num_replies);
var n=$("span.show_comments_link").hasClass("comments-closed"),m=parseInt(l.result.num_replies);
$(".show_comments_link").click(function(){$(".comment_master_list").toggleClass("slidedown");
var o=$(this);
o.text(o.text()=="Show comments."?"Hide comments.":"Show comments.")
});
if(m==1){$(".numberTag").text("There is");
$(".commentTag").text("comment")
}if(!m){$(".commentCount").text("no");
var k=(n)?"":"Be the first to comment.";
$(".show_comments_link").unbind("click").text(k).css({color:"#666",cursor:"default"});
$(".comment_master_list").removeClass("slideup").addClass("slidedown")
}if(n){if(m){$(".vf-comment-box").hide()
}else{$(".vf-commenting").hide()
}}}})
}$("div.facebook-share").on("click",function(k){if(typeof FB!="object"){return
}FB.ui({method:"feed",link:$(this).attr("data-href")},function(l){if(l&&l.post_id){facebookShare()
}})
});
$(".explore-this-story .icopyright a").on("click",function(l){l.preventDefault();
var k=$('<div class="email-lightbox"/>').append($('<div class="container float-clear"/>').append($('<div class="logobar float-clear"/>').append($('<div class="logo"/>').text("thestar.com")).append($('<div class="close-box"/>').text("Close"))).append($('<div class="article-info"/>').append($("<h2/>").text("Republish with iCopyright")))).append($('<div class="iframe-container"/>').append($("<iframe/>").attr("src",$(this).attr("href")).css("height",480)));
k.dialog({modal:true,width:548,autoOpen:false,resizable:false,draggable:false,zIndex:999999,create:function(m){$(m.target).parent().css("position","fixed");
$(m.target).parent().find(".ui-dialog-titlebar").remove()
},open:function(m,n){var o=$(this);
$(this).find(".close-box").on("click",function(){o.dialog("close")
})
},close:function(m,n){$(this).dialog("destroy").remove()
}}).dialog("open")
});
$(".reddit-button a").on("click",function(k){k.preventDefault();
if(typeof redditButton!="undefined"){redditButton()
}window.open("http://www.reddit.com/submit?url="+encodeURIComponent($(this).attr("data-url")))
});
$(".article-toolbar .print-article, .explore-this-story-items .print-article").on("click",function(){printArticle($(this).text());
window.open($(this).attr("data-printurl"))
});
$(".article-toolbar .text-increase, .article-toolbar .text-decrease, .explore-this-story-items .text-increase, .explore-this-story-items .text-decrease").on("click",function(){var k=parseInt($(".article-body > .feature div.article-story .article-story-body").css("font-size").replace("px",""),10);
k=k+($(this).hasClass("text-increase")?1:-1);
if(k>Article.defaultFontSize+4){k=Article.defaultFontSize+4
}if(k<Article.defaultFontSize-4){k=Article.defaultFontSize-4
}$(".article-body > .feature div.article-story .article-story-body").css("font-size",k+"px");
$.cookie("article-font-size",String(k),{expires:1095,path:"/"})
});
...
$(".article-toolbar .email-article, .explore-this-story-items .email-article").on("click",function(){emailArticle($(this).text());
var k=$(this).attr("data-emailurl");
$('<div class="email-lightbox"/>').load(k,function(){$(this).dialog({modal:true,width:462,autoOpen:false,resizable:false,draggable:false,zIndex:999999,create:function(l){var m=$(this);
$(l.target).parent().css("position","fixed");
$(l.target).parent().find(".ui-dialog-titlebar").remove();
$.getScript("https://www.google.com/recaptcha/api/js/recaptcha_ajax.js",function(n,p,o){Article.loadCaptcha()
});
$("form#sendtoafriend").on("submit",function(t){t.preventDefault();
var q=$(this);
q.find("fieldset.message").empty().hide().removeClass("error").removeClass("success");
var r=unescape($.trim(q.find("#storyHeadline").val()));
var n=unescape($.trim(q.find("#storySubheadline").val()));
var p="Story from the thestar.com: "+r;
var o=($.trim(q.find("#yourname").val()).length?$.trim(q.find("#yourname").val()):$.trim(q.find("#emailfrom").val()))+" has shared a story with you from thestar.com:";
$.ajax({type:"GET",url:q.find("#postUrl").val(),dataType:"json",data:{yourname:$.trim(q.find("#yourname").val()),emailfrom:$.trim(q.find("#emailfrom").val()),emailto:$.trim(q.find("#emailto").val()),emailbody:$.trim(q.find("#emailbody").val()),recaptcha_response_field:q.find("input#recaptcha_response_field").val(),recaptcha_challenge_field:q.find("input#recaptcha_challenge_field").val(),emailsubject:p,story_Headline:r,story_Subheadline:n,stroy_AbsoluteUrl:q.find("#storyAbsoluteUrl").val(),emailbodyintro:o},success:function(u){switch(u.msg){case"success":Article.setEmailMessage(q,"Your email has been sent successfully!","success");
q.find("fieldset.humans").remove();
setTimeout(function(){m.dialog("close")
},3000);
break;
case"email error":Article.loadCaptcha();
Article.setEmailMessage(q,"ERROR: please verify the info you provided is correct.","error");
break;
case"captcha error":Article.loadCaptcha();
Article.setEmailMessage(q,"ERROR: please try the ReCaptcha again.","error");
break
}},error:function(u,w,v){Article.loadCaptcha();
Article.setEmailMessage(q,"ERROR: the server encountered an issue. Please try again.","error")
}})
})
},open:function(l,m){var n=$(this);
$(this).find(".close-box").on("click",function(){n.dialog("close")
})
},close:function(l,m){$(this).dialog("destroy").remove();
Article.killCaptchaFrame()
}}).dialog("open")
})
});
$(".article-toolbar .error-article, .explore-this-story-items .error-article").on("click",function(){var k=$(this).attr("data-errorurl");
$('<div class="email-lightbox"/>').load(k,function(){$(this).dialog({modal:true,width:462,autoOpen:false,resizable:false,draggable:false,zIndex:999999,create:function(l){var m=$(this);
$(l.target).parent().css("position","fixed");
$(l.target).parent().find(".ui-dialog-titlebar").remove();
$.getScript("https://www.google.com/recaptcha/api/js/recaptcha_ajax.js",function(n,p,o){Article.loadCaptcha()
});
$("form#reportanerror").on("submit",function(p){p.preventDefault();
var n=$(this);
n.find("fieldset.message").empty().hide().removeClass("error").removeClass("success");
var o=unescape($.trim(n.find("#storyHeadline").val()));
$.ajax({type:"GET",url:n.find("#postUrl").val(),dataType:"json",data:{yourname:$.trim(n.find("#yourname").val()),emailfrom:$.trim(n.find("#emailfrom").val()),phone:$.trim(n.find("#phone").val()),emailbody:$.trim(n.find("#emailbody").val()),recaptcha_response_field:n.find("input#recaptcha_response_field").val(),recaptcha_challenge_field:n.find("input#recaptcha_challenge_field").val(),topic:$.trim(n.find("#issue").val()),story_Headline:o,stroy_AbsoluteUrl:n.find("#storyAbsoluteUrl").val()},success:function(q){switch(q.msg){case"success":Article.setReportErrorMessage(n,"Your email has been sent successfully!","success");
n.find("fieldset.humans").remove();
setTimeout(function(){m.dialog("close")
},3000);
break;
case"email error":Article.loadCaptcha();
Article.setReportErrorMessage(n,"ERROR: please verify the info you provided is correct.","error");
break;
case"captcha error":Article.loadCaptcha();
Article.setReportErrorMessage(n,"ERROR: please try the ReCaptcha again.","error");
break
}},error:function(q,t,r){Article.loadCaptcha();
Article.setReportErrorMessage(n,"ERROR: the server encountered an issue. Please try again.","error")
}})
})
},open:function(l,m){var n=$(this);
$(this).find(".close-box").on("click",function(){n.dialog("close")
...
$(".thestar-image .buyphoto-article").on("click",function(){var k=$(this).attr("data-buyphotourl");
$('<div class="email-lightbox"/>').load(k,function(){$(this).dialog({modal:true,width:462,autoOpen:false,resizable:false,draggable:false,zIndex:999999,create:function(l){var m=$(this);
$(l.target).parent().css("position","fixed");
$(l.target).parent().find(".ui-dialog-titlebar").remove();
$.getScript("https://www.google.com/recaptcha/api/js/recaptcha_ajax.js",function(n,p,o){Article.loadCaptcha()
});
$("form#buyarticlephoto").on("submit",function(q){q.preventDefault();
var o=$(this);
o.find("fieldset.message").empty().hide().removeClass("error").removeClass("success");
var p=unescape($.trim(o.find("#storyHeadline").val()));
var n=unescape($.trim(o.find("#storyImageCaption").val()));
$.ajax({type:"GET",url:o.find("#postUrl").val(),dataType:"json",data:{yourname:$.trim(o.find("#yourname").val()),emailfrom:$.trim(o.find("#emailfrom").val()),phone:$.trim(o.find("#phone").val()),photosize:o.find("#psize").val(),usefor:$("input[name=use]:checked").val(),recaptcha_response_field:o.find("input#recaptcha_response_field").val(),recaptcha_challenge_field:o.find("input#recaptcha_challenge_field").val(),story_Headline:p,story_AbsoluteUrl:o.find("#storyAbsoluteUrl").val(),story_ImageCaption:n,story_ImageUrl:o.find("#storyImageUrl").val(),story_ImageID:o.find("#storyImageID").val()},success:function(r){switch(r.msg){case"success":Article.setReportErrorMessage(o,"Thank you for your interest in Toronto Star photographs. You will be contacted during normal business hours to discuss placing an order.","success");
o.find("fieldset.humans").remove();
setTimeout(function(){m.dialog("close")
},10000);
break;
case"email error":Article.loadCaptcha();
Article.setReportErrorMessage(o,"ERROR: please verify the info you provided is correct.","error");
break;
case"captcha error":Article.loadCaptcha();
Article.setReportErrorMessage(o,"ERROR: please try the ReCaptcha again.","error");
break
}},error:function(r,u,t){Article.loadCaptcha();
Article.setReportErrorMessage(o,"ERROR: the server encountered an issue. Please try again.","error")
}})
})
},open:function(l,m){var n=$(this);
$(this).find(".close-box").on("click",function(){n.dialog("close")
...
if($("body.articlepage.type-story").length&&$("#comments-section").length&&$(".inside-thestar").length){var c=$(".inside-thestar").parent().offset();
var b=$("#comments-section").offset();
var a=$(".inside-thestar").parent().outerHeight(true);
var f=((c.top+a)-b.top);
if(f>0){$("#comments-section").css("margin-top",f+18)
...
},recordSiteCat:function(a){if(typeof imagesIds!="undefined"&&imagesIds.length&&typeof s.prop20!="undefined"&&s.prop20.length){var b=imagesIds.split(",");
var c=s.prop20;
SiteCatalyst.registerEvent({events:"event20",eVariables:"eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar9,eVar10,prop18,eVar18,prop19,eVar19,prop20,eVar20,prop32,eVar32",eValues:new Array(s.eVar2,s.eVar3,s.eVar4,s.eVar5,s.eVar6,s.eVar7,s.eVar8,s.eVar9,s.eVar10,s.prop18,s.eVar18,b[a],b[a],c,c,s.prop32,s.eVar32),title:"Image Click"})
}},openDialog:function(h,b,k,d,j,m,l){var o=h.find(".slideshow");
...
b(a,function(h){if(typeof h!=="undefined"&&typeof h.data!=="undefined"){data=h.data.split(":");
if(typeof myStar!="undefined"&&h.origin==myStar.domain&&data[0]=="myStar"){var d=(typeof data[1]!=="undefined")?data[1]:"";
var g=(typeof data[2]!=="undefined")?data[2]:"";
var f=(typeof data[3]!=="undefined")?data[3]:"";
if(d=="signedIn"){localStorage.setItem("signedIn","true");
localStorage.setItem("userId",g);
localStorage.setItem("userLoc",f);
trackMyStarEvent("event8","myStarSignIn")
}else{if(d=="signedOut"){localStorage.setItem("signedIn","false");
trackMyStarEvent("event9","myStarSignOut");
localStorage.setItem("userId",null);
localStorage.setItem("userLoc",null)
}else{if(d=="newUser"){localStorage.setItem("signedIn","true");
localStorage.setItem("userId",g);
localStorage.setItem("userLoc",f);
trackMyStarEvent("event8","myStarSignIn");
...
(function(j){var b=function(k){j.dbg("tracking: "+k)
};
if(typeof s_gi!="function"||s_account==undefined){b("s_gi not a function!");
return
}if((/\.bb\.html(?:\?.*)*$/).test(location.href)){s_account="thestarmobilewebphone"
}var f=s_gi(s_account),c=null;
var h=function(m,l){if(l==undefined){l=m
}var k={spot:"pev2",category:"eVar2"};
return(m in k)?k[m]:l
},g=function(n,m){var l=(this&&this.href)?this:true;
var q=j.extend({type:"o",event:"",spot:"",category:""},m),r=["events"];
for(var o in q){var k=h(o);
if(k.match(/^(?:prop|eVar|pev)\d+$/)&&q[o]){f[k]=q[o];
r.push(k);
b(k+" = "+f[k])
}}f.linkTrackVars=r.join(",");
if(q.event){f.events=f.linkTrackEvents=q.event
}f.tl(window,q.type,q.spot);
b("s.linkTrackVars = '"+f.linkTrackVars+"'")
},d=function(k,l){j(k).click(function(n){var o=n.target;
if(j(o).is("a *")){o=j(o).parent("a")[0]
}if(j(o).is("a")&&!j(o).is(".notrack")){var q="",r="",m="";
if(j(o).is(".paper-homepage")){q="Today's Front Page Box Click"
}else{q=j(k).attr("data-omni-source");
r=j(k).attr("data-omni-pos");
m=j(o).attr("data-omni-category")||"";
if(r){q+="_"+r
}}var p={spot:q,category:m,type:"o"};
if(l){p=j.extend(p,l)
}b("target: '"+o.tagName+"', props: "+j.repr(p));
g(o,p);
j(o).addClass("notrack");
if(j.isDebug()){return false
}}})
};
var a=function(){if(c==null){return
}if(document.activeElement==c){var k=j(c).attr("data-omni-source")||j(c).attr("id");
g(j(c).parent[0],{spot:k,type:"o"});
b("iframe clicked!");
c=null
}else{setTimeout(a,10)
}};
j.fn.trackLinks=function(k){return this.each(function(){d(this,k)
})
};
j.fn.trackIframe=function(){return this.each(function(){if(this.tagName!="IFRAME"){return
}var k=this;
j(this).bind({mouseenter:function(){c=k;
if(document.activeElement!=c){a()
}},mouseleave:function(){c=null
}})
})
};
j(function(){if(j.isDebug()){j("form.search").submit(function(k){k.preventDefault()
})
}j("iframe#fb-top-frame").trackIframe();
j(".home-sect").trackLinks();
j(".more-content-and-ads").trackLinks();
j(".header-search-submit").mousedown(function(k){trackSearchPage(k.target)
});
j(window).click(function(k){var l=j(k.target);
if(l.is(".open-user-widget")){trackMyStarEvent("","myStarSignInButton");
if(j.isDebug()){k.preventDefault()
}}})
})
})(jQuery);
function trackOmniture(d,c){var b=s_gi(s_account);
b.linkTrackVars="events";
for(var a in d){b[a]=d[a];
if(a.indexOf("prop")==0||a.indexOf("eVar")==0||a.indexOf("pev")==0){b.linkTrackVars+=","+a
}}b.tl(c||true,"o",b.linkname)
}function trackErrorPage(){var a=s_gi(s_account);
trackSectionEVarsForEvents(a);
a.trackExternalLinks=false;
a.eVar44="404";
a.linkTrackVars="eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar9,eVar10,eVar32,eVar44,events";
a.linkTrackEvents="event22";
a.events="event22";
a.tl(this,"o","error page");
a.events=""
}function trackSearchPage(b){var a=s_gi(s_account);
trackSectionEVarsForEvents(a);
a.trackExternalLinks=false;
a.prop36=a.eVar36=a.getQueryParam("q");
a.linkTrackVars="eVar1,eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar9,eVar10,eVar32,eVar36,events";
a.linkTrackEvents="event2";
a.events="event2";
a.tl(b||true,"o","SearchIconButton");
a.events=""
}function facebookShare(){var a=s_gi(s_account);
trackSectionEVarsForEvents(a);
a.eVar41="fShare";
a.events="event17";
a.trackExternalLinks=false;
a.linkTrackVars="eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar9,eVar10,eVar32,eVar41,events";
a.linkTrackEvents="event17";
a.tl(this,"o","fShare")
}function plusone(b){var a=s_gi(s_account);
trackSectionEVarsForEvents(a);
a.eVar41="GooglePlusOne";
a.events="event17";
a.trackExternalLinks=false;
a.linkTrackVars="eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar9,eVar10,eVar32,eVar41,events";
a.linkTrackEvents="event17";
a.tl(this,"o",a.eVar41)
}function twitterButton(){var a=s_gi(s_account);
trackSectionEVarsForEvents(a);
a.eVar41="Twitter";
a.events="event17";
a.trackExternalLinks=false;
...
a.linkTrackEvents="event17";
a.tl(this,"o",a.eVar41)
}function linkedInButton(){var a=s_gi(s_account);
trackSectionEVarsForEvents(a);
a.eVar41="LinkedIn";
a.events="event17";
a.trackExternalLinks=false;
...
a.linkTrackEvents="event17";
a.tl(this,"o",a.eVar41)
}function redditButton(){var a=s_gi(s_account);
trackSectionEVarsForEvents(a);
a.eVar41="Reddit";
a.events="event17";
a.trackExternalLinks=false;
...
a.linkTrackEvents="event17";
a.tl(this,"o",a.eVar41)
}function emailArticle(a){var b=s_gi(s_account);
trackSectionEVarsForEvents(b);
b.linkTrackEvents=b.events="event19";
b.trackExternalLinks=false;
b.linkTrackVars="eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar9,eVar10,prop14,eVar14,eVar32,events";
b.tl(this,"o",a)
}function printArticle(a){var b=s_gi(s_account);
b.linkTrackEvents=b.events="event18";
b.trackExternalLinks=false;
trackSectionEVarsForEvents(b);
b.linkTrackVars="eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar9,eVar10,prop14,eVar14,eVar32,events";
b.tl(this,"o",a)
}function trackHomePageTopFlip(c,b){var a=s_gi(s_account);
a.trackExternalLinks=false;
trackSectionEVarsForEvents(a);
a.prop43=a.eVar43=c;
if(b=="open"){a.linkTrackEvents=a.events="event3"
}else{if(b=="close"){a.linkTrackEvents=a.events="event4"
}}a.linkTrackVars="eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar9,eVar10,eVar32,prop43,eVar43,events";
a.tl(this,"o","HomePageTopFlip")
}function trackTopicTagClicks(a){var b=s_gi(s_account);
trackSectionEVarsForEvents(b);
b.trackExternalLinks=false;
b.prop34=b.eVar34=a;
b.linkTrackEvents=b.events="event15";
b.linkTrackVars="eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar9,eVar10,eVar32,prop34,eVar34,events";
b.tl(this,"o","TopicTagClick")
}function trackBreakingNewsClicks(){var a=s_gi(s_account);
trackSectionEVarsForEvents(a);
a.trackExternalLinks=false;
a.linkTrackEvents=a.events="event14";
a.linkTrackVars="eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar9,eVar10,eVar32,events";
a.tl(this,"o","BreakingNewsClick")
}function trackVideoEvents(a){var b=s_gi(s_account);
b.trackExternalLinks=false;
trackSectionEVarsForEvents(b);
b.linkTrackEvents=b.events=a;
b.linkTrackVars="channel,hier1,eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar9,eVar10,eVar32,eVar37,eVar38,eVar39,eVar45,eVar61,eVar62,events,products";
b.tl(this,"o","CustomLinkforVideo");
b.products="";
b.events=""
}function trackMyStarEvent(a,b){s.trackExternalLinks=false;
trackSectionEVarsForEvents(s);
s.linkTrackEvents=s.events=a;
...
s.tl(true,"o",b)
}function trackSectionEVarsForEvents(a){a.eVar1="s_code_v3.0 - 2014-10-20";
a.eVar32=a.wd.location;
if(a.pageName){a.hierDelim="|";
a.hier1=a.pageName.replace(/:/g,",");
a.hps=a.split(a.hier1,",");
a.channel=a.prop6=a.eVar6=a.prop2=a.eVar2=a.hps[0];
if(a.hps.length>1){a.prop3=a.eVar3=a.channel+a.hierDelim+a.hps[1];
a.prop7=a.eVar7=a.hps[1];
if(a.hps.length>2){a.prop4=a.eVar4=a.prop3+a.hierDelim+a.hps[2];
a.prop8=a.eVar8=a.hps[2];
if(a.hps.length>3){a.prop5=a.eVar5=a.prop4+a.hierDelim+a.hps[a.hps.length-1];
a.prop9=a.eVar9=a.hps[a.hps.length-1]
}}}else{a.prop2=a.eVar2=a.channel
}}}function trackPromoClick(d,f,b){var c=s_gi(s_account);
var a=d+"|"+f+"|"+b;
c.trackExternalLinks=false;
c.linkTrackVars="prop6,eVar6";
c.tl(this,"o",a)
}function trackArticleAAMFooter(b){var a=s_gi(s_account);
a.trackExternalLinks=false;
a.linkTrackVars="prop6,eVar6";
a.tl(this,"o",b)
...
!(function(f){var j=function(o,n){return(o!=null&&typeof o=="object"&&"JSON" in window)?(n?JSON.stringify(o,null,2):JSON.stringify(o)):""+o
},b=function(n){if("dbg" in f&&typeof f.dbg=="function"){f.dbg("bc_track_v3: "+j(n))
}};
if(!("brightcove" in window)){b("'brightcove' not found!");
return
}var l=null,k=("dfpConf" in window&&"videoSkip" in window.dfpConf&&window.dfpConf.videoSkip)?5:-1,a=function(){if(!l&&"s_gi" in window&&typeof s_gi=="function"&&"s_account" in window&&s_account){l=s_gi(s_account)
}return l
},m=function(q,o){var p=a();
if(p&&q){if(typeof q=="object"){for(var n in q){m(n,q[n])
}}else{p[q]=o
}}},h=function(){return""+("now" in Date)?Date.now():(new Date()).getTime()
},g={},d=function(n){if(n in g){g[n]=null;
delete g[n]
}},c=function(o){var n=("APIModules" in window)?window.APIModules:brightcove.api.modules.APIModules,p=brightcove.getExperience(o);
this.expId=o;
this.$obj=f("#"+o);
this.isRva=this.$obj.is(".rva");
this.modVP=p.getModule(n.VIDEO_PLAYER);
this.modAdv=p.getModule(n.ADVERTISING);
this.modExp=p.getModule(n.EXPERIENCE);
this.lastEvent="";
...
this.adUrl=this.buildPrerollUrl();
this.adPolicy=(this.adUrl)?{adServerURL:this.adUrl,prerollAds:true,prerollAdKeys:null}:null;
this.adIsOn=false;
this.adPaused=false;
this.throttle=null
};
c.prototype.buildPrerollUrl=function(){var p=("dfpConf" in window)?window.dfpConf:null;
if(!p){b("Global DFP config not found!");
return""
}var q={sz:(this.isRva)?p.rvaSize:p.videoSize,iu:p.path,ciu_szs:null,impl:"s",gdfp_req:1,env:"vp",output:"xml_vast2",vpos:"preroll",unviewed_position_start:1,referrer_url:document.referrer,description_url:window.location.href.split("?")[0],correlator:h()},o=[];
for(var n in q){if(null==q[n]){continue
}o.push(n+"="+encodeURIComponent(q[n]))
}return location.protocol+p.baseUrl.replace(/^https?\:/,"")+"?"+o.join("&")
};
c.prototype.updateVideoStatus=function(){var n=this.modVP.getCurrentVideo();
this.currentVideoTitle=n.displayName;
this.currentVideoId=n.id;
this.currentVideoRefId=("referenceId" in n&&n.referenceId)?n.referenceId:"unknown";
this.currentVideoSource=("customFields" in n&&"credit" in n.customFields&&n.customFields.credit)?n.customFields.credit:"unknown";
this.currentVideoUploaded=n.publishedDate
};
c.prototype.trackVideo=function(o){if(!("trackVideoEvents" in window)){return
}var n=a();
if(!n){return
}n.eVar37=this.currentVideoTitle;
n.eVar38=this.player;
n.eVar39=this.currentVideoId;
n.eVar45=this.currentVideoRefId;
n.eVar61=this.currentVideoSource;
n.eVar62=this.currentVideoUploaded;
window.trackVideoEvents(o)
};
c.prototype.setPreroll=function(){b("setting ad policy in "+this.expId+": "+j(this.adPolicy));
if(this.adPolicy){this.modAdv.setAdPolicy(this.adPolicy)
}};
c.prototype.logEvent=function(n){b(n.type+" fired in "+this.expId)
};
c.prototype.noThrottle=function(){if(this.throttle){clearTimeout(this.throttle)
}this.throttle=null
};
c.prototype.makeAdSkipOverlay=function(){var o=this,p=this.$obj.parent(),n=p.find(".skip-ad");
if(n.length){return
}p.css({position:"relative"});
f("<div/>").addClass("skip-ad").html("Skip Ad &rsaquo;").appendTo(p).fadeTo(0,0.5).one("click",function(){o.skipAd()
})
};
c.prototype.removeAdSkipOverlay=function(){b("removing Skip Ad");
this.$obj.siblings(".skip-ad").remove()
};
c.prototype.initAdSkip=function(){if(typeof k!="number"||k<0){b("Ad Skip disabled");
return
}var o=k*1000,n=this;
setTimeout(function(){if(n.adIsOn){n.makeAdSkipOverlay()
}},o)
};
c.prototype.skipAd=function(){if(this.adPaused){return
}this.modAdv.stopAd();
this.removeAdSkipOverlay()
};
c.prototype.setAdPause=function(n){var o=this.$obj.siblings(".skip-ad");
this.adPaused=!!n;
if(o.length){if(this.adPaused){o.hide()
}else{o.show()
}}};
c.getTracker=function(n){return(n in g)?g[n]:null
};
c.handleAdEvent=function(n,p){var o=c.getTracker(p);
if(!o){return
}if(n.type==o.lastEvent){return
}switch(n.type){case"adStart":o.adIsOn=true;
o.initAdSkip();
break;
case"adClick":o.skipAd();
break;
case"adPause":o.setAdPause(true);
break;
case"adResume":o.setAdPause(false);
break;
case"adComplete":o.adIsOn=false;
o.removeAdSkipOverlay()
}o.lastEvent=n.type;
o.logEvent(n)
};
c.handleMediaEvent=function(n,q){var p=c.getTracker(q);
if(!p){return
}if(n.type==p.lastEvent&&n.type!="mediaProgress"){return
}var o=true;
switch(n.type){case"mediaProgress":o=false;
if(p.throttle||p.isComplete){break
}if(n.position/n.duration>0.94){p.trackVideo("event7");
p.isComplete=true
}else{p.roundPos=Math.floor(n.position);
if(p.lastPosition!=p.roundPos&&p.roundPos%15==0){p.lastPosition=p.roundPos;
m("products",";;;;event6=15");
p.trackVideo("event6")
}}p.throttle=setTimeout(function(){p.noThrottle()
},1000);
break;
case"mediaChange":p.adPaused=p.adIsOn=false;
p.setPreroll();
p.isPlaying=false;
p.updateVideoStatus();
p.playVdoCount++;
break;
case"mediaBegin":p.noThrottle();
p.isComplete=false;
p.adIsOn=false;
p.updateVideoStatus();
p.trackVideo("event5");
break;
case"mediaPlay":if(p.isPlaying){break
}p.adIsOn=false;
p.isComplete=false;
p.lastPosition=0;
p.isPlaying=true;
break;
case"mediaComplete":p.isPlaying=false;
p.noThrottle();
break
}if(o){p.logEvent(n)
}p.lastEvent=n.type
};
c.onTemplateLoaded=function(p){if(f("#"+p).is(".adplayer, .rva")){b("ignoring ad player '"+p+"'");
return
}b("executing bcTrack.onTemplateLoaded in '"+p+"'");
var n,o=new c(p);
n="mediaBegin, mediaChange, mediaComplete, mediaError, mediaPlay, mediaProgress, mediaStop";
f.each(n.split(/\W+/),function(q,r){o.modVP.addEventListener(r,function(t){c.handleMediaEvent(t,p)
})
});
b("binding "+n+" in "+p);
n="adClick, adStart, adComplete, adPause, adResume";
f.each(n.split(/\W+/),function(q,r){o.modAdv.addEventListener(r,function(t){c.handleAdEvent(t,p)
})
});
b("binding "+n+" in "+p);
g[p]=o
};
window.onTemplateLoaded=c.onTemplateLoaded
})(jQuery);
...
var EchoComm={char_limit:50,commentsArray:{YourComments:new Array(),PopularComments:new Array(),MyComments:new Array()},displayComment:function(a){return a.length>EchoComm.char_limit?a.substring(0,EchoComm.char_limit).split(" ").slice(0,-1).join(" ")+"&hellip;":a
},createCommentObj:function(f,d,h,g,b,c){var a=new Object();
a.title=f;
a.uri=d;
a.comments=new Array();
commentPost=new Object();
commentPost.pubdate=b;
commentPost.username=g;
commentPost.comment=h;
a.comments.push(commentPost);
EchoComm.commentsArray[c].push(a);
return true
},getComments:function(w,h,d){var m=w.entries||[];
for(var t=0;
t<m.length;
t++){var b=m[t];
var l=false;
if((typeof b.object.context[0].title!=="undefined")&&(b.object.context[0].title.length>0)){if(EchoComm.commentsArray[h].length==0){l=EchoComm.createCommentObj(b.object.context[0].title,b.object.context[0].uri,b.object.content,b.actor.title,b.object.published,h)
}else{l=false;
for(commentObj in EchoComm.commentsArray[h]){if(EchoComm.commentsArray[h][commentObj].uri==b.object.context[0].uri){commentPost=new Object();
commentPost.pubdate=b.object.published;
commentPost.username=b.actor.title;
commentPost.comment=b.object.content;
EchoComm.commentsArray[h][commentObj].comments.push(commentPost);
l=true;
break
}}}if(!l){EchoComm.createCommentObj(b.object.context[0].title,b.object.context[0].uri,b.object.content,b.actor.title,b.object.published,h)
}}}switch(h){case"YourComments":var p=EchoComm.commentsArray[h].length;
var n=d.attr("data-comment-numofarticles");
var k=d.attr("data-comment-items");
var r=$("<ul/>");
if(p>0){for(var f=0;
(f<p&&f<n);
f++){var x=EchoComm.commentsArray[h][f].comments.length;
var g=$("<li/>");
g.append($('<p class="article"/>').append($('<a href="'+EchoComm.commentsArray[h][f].uri+'">'+EchoComm.commentsArray[h][f].title+"</a>")));
for(var v=0;
(v<x&&v<k);
v++){g.append($('<p class="quote"/>').append('"'+EchoComm.displayComment(EchoComm.commentsArray[h][f].comments[v].comment)+'"'))
}g.append($('<p class="post"/>').append($('<a href="'+EchoComm.commentsArray[h][f].uri+'">Post a comment</a>')));
r.append(g)
}d.find("p.loading").replaceWith(r.children("li").last().addClass("last-child").end().end())
}else{d.find("p.loading").html("No comments found!")
}break;
case"PopularComments":var x=0;
if(EchoComm.commentsArray[h].length){x=EchoComm.commentsArray[h][0].comments.length
}var k=d.attr("data-comment-items");
var r=$("<ul/>").addClass("article-inset-list").addClass("comment-list");
if(x>0){for(var v=0;
(v<x&&v<k);
v++){var y=new XDate(EchoComm.commentsArray[h][0].comments[v].pubdate,false).toString("MMM d, h(:mm) tt");
var u=$("<li/>");
u.append($('<p class="quote"/>').append('"'+EchoComm.displayComment(EchoComm.commentsArray[h][0].comments[v].comment)+'"'));
u.append($('<p class="source"/>').append("<strong>"+EchoComm.commentsArray[h][0].comments[v].username+"</strong>").append("<span>"+y+"</span>"));
r.append(u)
}d.find("p.loading").replaceWith(r.children("li").last().addClass("last-child").end().end())
}else{d.find("p.loading").html("No comments found!")
}break;
case"MyComments":var p=EchoComm.commentsArray[h].length;
var r=$('<div id="commentswrapper">');
if(p>0){for(var f=0;
f<p;
f++){var x=EchoComm.commentsArray[h][f].comments.length;
var g=$('<div class="combox">');
var q='<a href="'+EchoComm.commentsArray[h][f].uri+'" target="_blank">'+EchoComm.commentsArray[h][f].title+"</a>";
for(var v=0;
v<x;
v++){var y=new XDate(EchoComm.commentsArray[h][f].comments[v].pubdate,false).toString("MMM d, h(:mm) tt");
var o=$('<div class="comment_row" data-comment_url="#">');
var a=$('<p class="comment">'+EchoComm.commentsArray[h][f].comments[v].comment+"</p>");
var j=$('<p class="meta">Posted to: '+q+"<br />"+y+".</span></p>");
o.append(a);
o.append(j);
g.append(o)
}r.append(g)
}}else{r.append('<p class="no_comments">You haven&rsquo;t posted any comments yet.</p>')
}d.append(r);
break
}},fetchComments:function(c,a,b){var h=c;
var d=a;
var g=b;
if(h!=null){var f=$.ajax({url:h,cache:false,dataType:"jsonp",type:"GET",success:function(j){EchoComm.getComments(j,d,g)
}})
}}};
$(document).on("star_component_init",function(){$("div.echo-comm-container[data-init-me]").each(function(){EchoComm.fetchComments($(this).attr("data-comment-path"),$(this).attr("data-type"),$(this))
...
$(document).ready(function(){$(document).on("click","div.promo a",function(){var b=$(this).closest(".promo");
var c=b.attr("data-comp-path");
var d=b.find("a").index($(this))+1;
var a=($(this).attr("href")).replace(/http(s)?:\/\//gi,"");
if(a.indexOf(localprefix)!=-1){a=(a).replace(/.*\/(.*)$/gi,"$1").replace(".html","")
}trackPromoClick(c,d,a)
})
});
...
$(document).on("star_component_init",function(){if(typeof twttr!="undefined"&&typeof twttr.widgets!="undefined"){twttr.widgets.load()
}});
var topic_count=21;
...
var h=$("<div>");
$.each(j.hits,function(p,o){var v=$("<li>").addClass("float-clear");
if(o.hasImages){v.addClass("with-thumbnail");
var r=$("<a>").addClass("thumbnail icon-overlay").attr("href",o.path);
r.html($("<div>").html(o.icon).text());
var q=$("<img>").attr("src",o.imgPath).attr("alt",o.title);
r.append(q);
v.append(r)
}var w=$("<p>").addClass("headline");
var m=$("<a>").attr("href",o.path);
if(!o.hasImages){m.html($("<div>").html(o.icon).text())
}m.append(o.title);
w.append(m);
v.append(w);
var u=$("<p>").addClass("timestamp").html(o.timestamp);
v.append(u);
var l=$("<div>").addClass("abstract");
if(o.hasComments){var t=$("<span>").addClass("with-comment-icon").html(o["abstract"]);
l.append(t);
var n=$("<span>").addClass("comment-icon");
var k=$("<a>").attr("href",o.path).html("comments");
n.append(k);
l.append(n)
}else{l.html(o["abstract"])
}v.append(l);
h.append(v)
});
...
$(document).ready(function(){$(document).on("click","div.article-aam-footer a",function(d){var b=new Array();
var g=$(this).closest("div.article-aam-footer");
var c=document.location.pathname;
if($(this).parent().hasClass("comment-icon")){b.push("comment-icon")
}if($(this).hasClass("thumbnail")){b.push("thumbnail")
}if(typeof $(this).closest("li").attr("data-segment")!="undefined"){b.push("segment:"+$(this).closest("li").attr("data-segment"))
}var a=($(this).attr("href")).replace(/http(s)?:\/\//gi,"");
if(a.indexOf(localprefix)!=-1){a=(a).replace(/.*\/(.*)$/gi,"$1").replace(".html","")
}var f=["ArticleAAMFooter","page:"+c,"url:"+a];
if(b.length){f.push(b.join("|"))
}trackArticleAAMFooter(f.join("|"))
})
});
...
function stripAccents(c){var a=/[àáâãäçèéêëìíîïñòóôõöùúûüýÿÀÁÂÃÄÇÈÉÊËÌÍÎÏÑÒÓÔÕÖÙÚÛÜÝ]/g;
...
trackOmniture({events:"Event31",prop49:q.name,eVar49:q.name,prop48:"councillorview",eVar48:"councillorview",pev2:"councillorview",linkname:"link",linkTrackEvents:"Event31",pageName:"policard|"+q.searchname+"|councillor",prop2:"policard",eVar2:"policard",prop3:q.searchname,eVar3:q.searchname,prop4:"policard|"+q.searchname+"|councillor",eVar4:"policard|"+q.searchname+"|councillor"});
return false
})
});
...
trackOmniture({events:"Event32",prop49:$(this).data("councillorname"),eVar49:$(this).data("councillorname"),prop48:"councillorview",eVar48:"councillorview",pev2:"councillorview",linkname:"widget",linkTrackEvents:"Event32",pageName:"policard|"+$(this).data("councillor")+"|councillor",prop2:"policard",eVar2:"policard",prop3:$(this).data("councillor"),eVar3:$(this).data("councillor"),prop4:"policard|"+$(this).data("councillor")+"|councillor",eVar4:"policard|"+$(this).data("councillor")+"|councillor"});
return false
});
...
}}(function(a,b){a.type="text/javascript";
a.async=!0;
a.src=location.protocol!=="https:"?"//cdn.viafoura.net/vf.js":"//api.viafoura.com/app/js/vf.js";
b.parentNode.insertBefore(a,b)
}(document.createElement("script"),document.getElementsByTagName("script")[0]));
window.vfAsyncInit=function(){window.viafoura.ready(function(){window.viafoura.subscribe("onLoginSuccess",function(){$("button.vf-comment-submit").click(function(){trackMyStarEvent("event12","CommentSubmitted")
})
});
if(typeof myStar!="undefined"){myStar.getCurrentUser(function(a){if(a==null){$("#comments-section").addClass("loggedOut");
window.viafoura.session.logout()
}else{$("#comments-section").addClass("loggedIn")
}$(".viafoura").slideDown();
...
$(window).on("load",function(){$(".vf-comment-box > .vf-comment-user > .vf-avatar-container img.vf-avatar").first().click(function(){return false
});
if(typeof myStar!="undefined"){myStar.getCurrentUser(function(a){if(a==null){$(".vf-commenting").prepend('<a href="#" class="alt-vf-login">Login</a>');
listenSelector=".alt-vf-login, .vf-comment-form";
if($.browser.msie==true&&$.browser.version=="8.0"){listenSelector="#comments_list"
}$(listenSelector).click(function(){$("html, body").animate({scrollTop:0},600);
$(".mystar .settings a").trigger("click");
return false
})
}})
}});
(function(c){if(location.hash&&location.hash.indexOf("vf-")==1&&c("#comments-section").length){c.dbg("@@@ Hash: "+location.hash);
var a=location.hash.replace(/^\D+/,""),b=null,d=1000,f=function(){var j=c(".vf-comment-container");
for(var h=0,k=j.length;
h<k;
h++){if(c(j[h]).attr("data-id")==a){c.dbg("@@@ Found element with data-id = "+a);
b=j[h];
break
}}if(b){var g=c(b);
g.addClass("high").parents(":hidden").show();
g.one("mouseenter",function(){c(this).removeClass("high")
});
if(g.is(":hidden")){g.show()
}b.scrollIntoView()
}else{setTimeout(f,d)
}};
c(function(){setTimeout(f,d)
})
}})(jQuery);
...
$(document).ready(function(){$.ajax({url:"http://www.workopolis.com/jobsearch/en/quicksearch-torstar",type:"GET",crossDomain:true,dataType:"jsonp",success:function(a){$("#search-container-partner").html(a)
...
$(function(){if(window.location.pathname.match(/^.*videos?\.html$/)){var a={videoID:$.getQueryParam("bctid","")},d=$("div.sectionplaylistvideo div.placeholder, div.feature div.html5-container");
for(var b in a){if(a[b]==""){delete a[b]
}}if(a.videoID&&d.length){var f=d.find("object").detach();
d.attachBrightcove(a).append($("<p>&nbsp;</p>")).append(f).css("height","auto")
}}$(".bc-secondary-video").each(function(){var g=$(this).attr("data-video-id")||"",h=0;
if(g&&g.match(/^\d+$/)){$(this).attachBrightcove({id:"secondary-video-"+(++h),videoID:g})
}});
setTimeout(function(){if($(".splitbox").length&&!$(window).hasAutoStartBrightcove()){$.dbg("auto-starting player not found!");
var g=[];
$(".splitbox").each(function(){var h=$(this).attr("src");
if(h&&$.inArray(h,g)<0){$("<script/>").attr("type","text/javascript").attr("src",h).attr("async","true").appendTo($("body"));
g.push(h)
}})
}else{$(".splitbox").remove()
}},1000);
var c=$(".rightrailrvaplaylistvideo .BrightcoveExperience");
if(c.length){setTimeout(function(){var h=$(".rightrailrvaplaylistvideo .BrightcoveExperience");
if(!h.length){return
}var k=h.attr("id"),j=brightcove.getExperience(k),g=(j)?j.getModule(APIModules.VIDEO_PLAYER):null;
if(!g){return
}h.parent().one("mouseenter",function(){$.dbg("### Setting volume to 0.5 in player "+k);
g.setVolume(0.5)
})
},1000)
}});
...
$(document).on("star_component_init",function(){$("div.article-keywords a").on("click",function(){var a=$(this).text();
trackTopicTagClicks(a)
})
});
$(document).on("star_component_init",function(){$("div.breaking-news a").on("click",function(){var a=$("div.breaking-news a").attr("href");
if(a!==typeof undefined&&a!==false){trackBreakingNewsClicks()
}})
});
