...
function postToFeed(){
	...
	mysqli_query($conn,"INSERT INTO edits (OutputLink,OriginalLink,EditIn,EditOut,title,VideoSource,SingleOrMulti) 
	VALUES ('".$html."','".$ytLink."','".$videoStart[0]."','".$videoStop[0]."','".$ytVideoTitle."','".$VideoSource."','".$SingleOrMulti."')");
	...
	<html>
		...
		<div class="dashboard-part">		
				...
				<span class="glyphicon glyphicon-link" aria-hidden="true"> </span><span id="copy-button-text"> Double Click To Copy</span></button>
						...
						<button type="button" class="btn btn-default bg-dark" id="postToFeed" onclick="">
		...
		var userViewMode=0;
		var vidDuration;
		var inPoint =['.singleOrMultiStartValues().',null];
	
		var outPoint =['.singleOrMultiStopValues().',null];
		var vidDuration;
		var currentCut=0;
	...
	jwplayer("JWvid").onTime(function (event){
		...
		if 	(userViewMode>-1){
			if (currentCut == outPoint.length && realVal > outPoint[currentCut]){
...
function templateVideoHTML() {
	...
	<html>
		...
		$(function () {
			for (var i=0;i<=(outPoint.length-2);i++){
				document.getElementById("cut"+i+"-tableElement").style.display="";
			}
		})
	...
	jwplayer("JWvid").onTime(function (event){
		...
		if 	(userViewMode>-1){
			if (currentCut >= (outPoint.length-2) && realVal > outPoint[currentCut]){
...
function singleOrMultiStartValues(){
	$videoStartData = ($_GET['InTimeCode']);
	

	parse_str($videoStartData);
	
	if (count($videoStart)>1){
		return implode(",", $videoStart);
		
	}else {
		return $videoStartData;
	}
}

function singleOrMultiStopValues(){
	
	$videoStopData = ($_GET['OutTimeCode']);

	
	parse_str($videoStopData);
	if (count($videoStop)>1){
		return implode(",", $videoStop);
	}else {
		return $videoStopData;
	}
}
...
<html>
	...
	var playerInstance = jwplayer("JWvid");
...
function assignVideoPlayback (editNumber){
	userViewMode=2;
	currentCut=editNumber;
	playerInstance.seek(inPoint[currentCut]);
}

playerInstance.on("play",function (event){
	if (userViewMode==0){
	playerInstance.seek(inPoint[0]);
	userViewMode=2;
	}
	vidDuration=playerInstance.getDuration();
