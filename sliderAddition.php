...
<body>
	<div class="navbar navbar-inverse navbar-fixed-top" role="navigation" style="padding-left:4.7%">
			...
			jwplayer("videoPlaybackFrame").onTime(function (event){
				...
				videoProgress = (realVal/vidDuration);
				videoProgress = videoProgress*100;
				videoProgress = videoProgress.toPrecision(2);
				...
				if (sliderIn < (realVal-15) || $( "#slider-range" ).slider( "values", 0 ) > (realVal+15) && userEditCounter == 0){
					
					jwplayer("videoPlaybackFrame").seek($( "#slider-range" ).slider( "values", 0 ));
					inPoint = sliderIn;
					userEditCounter=1;
				} 
				else if (sliderOut < (realVal-15) || sliderOut > (realVal+15) && sliderOut != 245 && userEditCounter == 1){
					
					jwplayer("videoPlaybackFrame").seek(sliderIn);
					outPoint=sliderOut;
					userEditCounter=2;
					
				} // else if (sliderIn < inPoint || sliderIn > inPoint && sliderIn != realVal){
// 					inPoint=sliderIn;
// 					userEditCounter=1;
// 				} else if (sliderOut < outPoint || sliderOut > outPoint && sliderOut != vidDuration){
// 					outPoint=sliderOut;
// 					userEditCounter=2;
// 				}
				// else if (userEditCounter==0){
// 					sliderIn = realVal;
//
...
// 				}
		...
		<div class="row">
		  ...
		  <div class="progress-bar progress-bar-info" id="videoProgressBar" role="progressbar" style="">
		</div>
		</div>
		</div>
		<div class="row">
		<p>
		  <label for="sliderAmount">Timecode:</label>
		  <input type="text" id="sliderAmount" readonly style="border:0; color:#f6931f; font-weight:bold;">
		</p>
		...
		<div class="row">
			<div class="col-md-8 col-md-offset-3" id="videoPlayerButtons" >
				...
				<button type="button" class="btn btn-default btn-lg" id="playPauseButton" onclick="playPauseVideo();" style="display:none">
					<span class="glyphicon glyphicon-pause" id="playPauseSpan" aria-hidden="true"></span>
				</button>
